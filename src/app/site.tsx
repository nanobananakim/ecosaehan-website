...
export function Header() {
  ...
  return <header className="site-header"><div className="nav-wrap"><Logo /><nav className={menuOpen ? "nav-links open" : "nav-links"}>
    ...
    <Link className="home-link admin-link" href="/admin" onClick={closeMenu}><Lock className="admin-lock-icon" size={17} strokeWidth={2.4} />관리자</Link>
...
const faqItems = [
  ...
  {
    category: "수목천연밴드",
    question: "수목천연밴드는 나무 이식 후 어느 정도 후에 땅에서 완전히 분해되나요?",
    answer: "토양 속에서 완전히 분해되는 기간은 보통 약 1~3년 정도로 보는 게 현실적입니다. 다만 두 소재의 분해 속도가 다릅니다. 면 섬유는 토양의 미생물·수분·온도 조건이 좋으면 수개월~1년 정도부터 강도가 크게 떨어질 수 있고, 천연라텍스는 상대적으로 더 오래 남아 전체 밴드가 육안으로 거의 사라지는 데는 1~3년 이상 걸릴 수 있습니다. 대략적으로 보면 0~6개월은 결속 기능 유지 → 6~12개월부터 섬유 및 라텍스 열화 진행 → 1~2년 사이 상당 부분 약화·분해 → 약 2~3년 전후 대부분 자연분해 정도를 기본 예상치로 잡는 게 합리적입니다."
  },
  {
    category: "화물차탄력바",
    question: "제품마다 어떤 특징이 있나요?",
    answer: "고탄력바 50mm, 65mm가 가장 기본형이고, 초강력탄력바와 신초강력탄력바는 열전사가 들어갑니다. 소재 면에서도 차이가 있는데, 고탄력바는 일반 폴리에스테르가 들어가지만, 초강력탄력바와 신초강력탄력바는 폴리 고강력사가 사용됩니다. 초강력탄력바와 신초강력탄력바의 차이는 라텍스 굵기입니다. 신초강력탄력바에 들어가는 라텍스가 가장 굵어서, 탄성이 더 강하고 인장강도가 더 우수합니다."
  },
  {
    category: "지주목결속바",
    question: "지주목결속바를 사용하는 이유는 무엇인가요?",
    answer: "예전에는 반생이나 녹화끈, 로프류로 지주목과 결속했습니다. 하지만 지주목결속바는 원사와 라텍스가 함께 들어가 있어 탄성이 있고, 큰 힘을 들이지 않고도 몇 바퀴만 감으면 훨씬 짱짱하게 고정된다는 장점이 있습니다."
  }
];
